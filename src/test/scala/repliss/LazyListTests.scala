...
//    assert(List(0, 1, 1, 2) == fibs.take(4).toList)

    var lazeCount = "0"
    def lazeL(i: Int) = { println(s"access($i)"); lazeCount += i; i }
    def e = { println("access e"); lazeCount += "e"; LazyList.empty }
    val xs21 = lazeL(1) #:: ((lazeL(2) #:: lazeL(3) #:: e))
  ...
  @Test
  def t8680: Unit = {
    def pre(n: Int) = LazyList.from(-n to -1)

    def cyc(m: Int) = {
      lazy val s: LazyList[Int] = LazyList.from(0 until m) #::: s
      s
    }
